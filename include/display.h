...
// Immediately clears and pushes the matrix so off-state pixels update at once.
// Sets ledColourChanged so mode loops can repaint their on-state pixels.
void setLedColours(uint8_t onR, uint8_t onG, uint8_t onB,
                   uint8_t offR, uint8_t offG, uint8_t offB);

// Set by setLedColours(); cleared by each mode loop after it repaints on-pixels.
